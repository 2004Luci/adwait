...
export async function POST(request: NextRequest) {
  try {
    ...
    if (selectedDate <= now) {
      return NextResponse.json({ message: "Selected date must be in the future" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Consultation scheduled successfully" }, { status: 200 });
  } catch (error) {
